...
export interface SharedProps {
  captchaTopics?: string[]; // CAPTCHA topics. Pseudorandom, default values will
  // be used if captchaTopics is not defined.
  cellsTall?: number; // Number of cells in each column. If not defined, column heights will be
  // equal to row lengths.
  ...
  imgTopicUrls: ImgTopicType[]; // Array of image URLs with associated tags. The tags are
  ...
  // denied access. Must be greater than or equal to minAttempts.
  onClickVerify?: () => void; // Function to execute on clicking 'Verify'.
  onMaxAttempts?: () => void; // Function called when the user has met the maximum
  // number of attempts.
...
export interface CaptchaButtonProps extends SharedProps {
  ...
  onClickCheckbox?: () => void; // Executes on clicking the checkbox, does not execute if the
  // CAPTCHA popup is open.
